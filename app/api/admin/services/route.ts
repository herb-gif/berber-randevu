...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({} as any))) as any;
    ...
    if (!id) {
      ...
      const durRaw = body.default_duration_min;
      const default_duration_min = Number(durRaw);
      if (!Number.isFinite(default_duration_min) || default_duration_min <= 0) {
      ...
      const insertRow: Record<string, any> = {
        name,
        service_type,
        default_duration_min: Math.round(default_duration_min),
      };
