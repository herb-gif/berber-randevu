    ...
    <main className="min-h-screen bg-neutral-50 p-6">
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">Hizmet Fiyatları</h1>
        ...
        <div className="rounded-2xl border bg-white shadow-sm overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading &&
                !unauth &&
                rows.map((r) => <ServiceRow key={r.id} row={r} onSave={save} />)}
            </tbody>
          </table>
        </div>
...
function ServiceRow({ row, onSave }: { row: Row; onSave: (id: string, price: number) => Promise<void> }) {
  ...
  return (
    <tr className="border-t">
      ...
      <td className="p-3">{row.default_duration_min} dk</td>
      <td className="p-3">
        <input
      ...
      <td className="p-3 text-right">
        <button className="rounded-lg bg-black px-3 py-2 text-white" onClick={() => onSave(row.id, price)}>
          Kaydet
        </button>
      </td>
    </tr>
