...
export default function AdminManualAppointmentPage() {
  ...
  const [blockResource, setBlockResource] = useState<"hair" | "niyazi" | "external">("hair");
  ...
  async function createBlock() {
    if (!date) return setToast("Tarih seç");
    if (!blockTime.trim()) return setToast("Saat seç");
    if (!blockDurationMin || blockDurationMin <= 0) return setToast("Süre seç");
    if (blockResource === "hair" && !blockBarberId) return setToast("Berber seç");
    ...
    try {
      const res = await fetch("/api/admin/blocks", {
        ...
        body: JSON.stringify({
          ...
          barber_id: blockResource === "hair" ? blockBarberId : null,
          reason: blockReason,
          note: blockNote,
        }),
      });
      ...
      if (!res.ok) {
        if (res.status === 409) {
          return setToast(data.error || "Bu saat zaten dolu veya bloklu.");
        }
      ...
      setBlockTime("");
      setBlockDurationMin(60);
      setBlockReason("mola");
      setBlockNote("");
      if (blockResource !== "hair") setBlockBarberId("");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-neutral-950 p-6 text-neutral-100">
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <div className="inline-flex rounded-2xl border border-white/10 bg-neutral-900 p-1 shadow-sm">
          <button
            type="button"
            ...
            className={`rounded-xl px-4 py-2 text-sm transition ${
              mode === "manual"
                ...
                : "text-white/70 hover:text-neutral-100"
            }`}
          >
            Manuel Randevu
          </button>
        ...
        {mode === "manual" ? (
                    ...
                    <label
                      ...
                      className={`flex items-center justify-between rounded-xl border p-3 transition ${
                        checked
                          ...
                          : "border-white/10 bg-neutral-900/80 hover:bg-neutral-800/80"
                      }`}
                    >
                ...
                <div className="mt-2 space-y-2">
                  {laserOptions.map((o) => {
                    ...
                    return (
                      <label
                        key={o.id}
                        className="flex items-center justify-between rounded-xl border border-white/10 bg-neutral-900/80 p-3"
                      >
                        <span className="flex items-center gap-3">
                          <input
                            type="checkbox"
                    ...
                    <button
                      ...
                      className={`rounded-xl border px-3 py-2 text-sm ${
                        picked === s
                          ...
                          : "bg-neutral-900 text-neutral-100 border-white/10 hover:bg-neutral-800"
                      }`}
                    >
                      {s}
                    </button>
          ...
          <div className="rounded-2xl border border-white/10 bg-neutral-900 p-4 shadow-sm space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-neutral-100">Saat</div>
                <input
                  type="time"
                  className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
                  value={blockTime}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <div className="text-sm font-semibold text-neutral-100">Kaynak</div>
                <select
                  className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
                  value={blockResource}
                  onChange={(e) => setBlockResource(e.target.value as any)}
                >
              ...
              <div>
                <div className="text-sm font-semibold text-neutral-100">Süre</div>
                <select
                  className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
              ...
              <div>
                <div className="text-sm font-semibold text-neutral-100">Sebep</div>
                <select
                  className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
                  value={blockReason}
            ...
            <div>
              <div className="text-sm font-semibold text-neutral-100">Not</div>
              <textarea
                className="mt-2 min-h-[96px] w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
                value={blockNote}
            ...
            <div className="flex items-end justify-end">
              <button
                onClick={createBlock}
                disabled={saving}
                className="rounded-xl px-4 py-2 bg-mc-black text-mc-bronze border border-mc-bronze hover:bg-mc-bronze hover:text-neutral-100 transition disabled:opacity-50"
              >
                {saving ? "Kaydediliyor…" : "Saati Kapat"}
              </button>
            </div>

            <div className="text-xs text-white/50">
              Not: Bu alan müşteri oluşturmadan zamanı satışa kapatır. Çakışma varsa sistem izin vermez.
            </div>
          </div>
        )}
